...
@router.get("/filter")
async def get_filtered_rdvcompdh(current_user: dict = Depends(auth_dependency)):
    try:
        if isinstance(current_user, JSONResponse):
            return current_user
        ...
        with get_db() as (conn, cursor):
            cursor.execute(
                ...
                SELECT 
                    ...
                    tenantid, bkcarea, createdate, compshortname,
                    user_email, version
                FROM tst1a.datasets as dt
                JOIN tst1a.datastores as ds on dt.datastoreshortname = ds.dsshortname
                WHERE ds.dstype = 'postgresql'
                ORDER BY datasetshortname ASC, dataproductshortname ASC, projectshortname ASC 
                """
            )
            rdvcompdh_list = []
            for rdv in cursor.fetchall():
                ...
                rdvcompdh_list.append(rdvcompdh_dict)
        return response(200, "filtered RdvCompDh fetched successfully", data=rdvcompdh_list)
    except Exception as e:
        return response(400, str(e))
